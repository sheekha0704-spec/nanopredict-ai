...
def load_and_prep(uploaded_file=None):
    ...
    models = {col: GradientBoostingRegressor(n_estimators=100, random_state=42).fit(X, df_train[f'{col}_clean']) for col
              in targets}
    df_train['is_stable'] = df_train.get('Stability', pd.Series(['stable'] * len(df_train))).str.lower().str.contains(
        'stable').astype(int)
...
with st.sidebar:
    ...
    step_choice = st.radio("Formulation Steps",
                           ["Step 1: Drug & Data", "Step 2: Advanced Solubility", "Step 3: Component AI & Ratios",
                            "Step 4: Phase Ratios", "Step 5: Finalize", "Step 6: Analysis"])
...
if step_choice == "Step 1: Drug & Data":
    ...
    with col1:
        ...
        if mol:
            st.session_state.logp, st.session_state.mw = round(Descriptors.MolLogP(mol), 2), round(
    ...
    with col2:
        if le_dict:
            ...
            # Populated Oil Data
            oils = ["MCT Oil", "Oleic Acid", "Isopropyl Myristate", "Capmul MCM", "Labrafil M", "Castor Oil",
            ...
            aff_df = pd.DataFrame({"Oil": oils, "Solubility (%)": scores}).sort_values("Solubility (%)",
                                                                                       ascending=False)
            st.plotly_chart(
                px.bar(aff_df, x="Solubility (%)", y="Oil", range_x=[0, 100], orientation='h', color="Solubility (%)",
...
# --- STEP 3: COMPONENT AI ---
    ...
    with col1:
        st.session_state.km_ratio = st.select_slider("Smix Ratio (Surfactant : Co-Surfactant)",
                                                     options=["1:1", "2:1", "3:1", "4:1"], value="2:1")
        st.session_state.oil_choice = st.selectbox("Select Target Oil",
                                                   sorted(le_dict['Oil_phase'].classes_) if le_dict else ["None"])
    with col2:
    ...
    if df_raw is not None:
        ...
        if not recs.empty:
            ...
            st.success(
                f"**AI Prediction:** Based on a {st.session_state.km_ratio} ratio, the optimal surfactant is **{best_s}**.")
...
# --- STEP 5: FINALIZE ---
    ...
    st.session_state.s_final = st.selectbox("Confirm Surfactant",
                                            sorted(le_dict['Surfactant'].classes_) if le_dict else ["None"])
    st.session_state.cs_final = st.selectbox("Confirm Co-Surfactant",
                                             sorted(le_dict['Co-surfactant'].classes_) if le_dict else ["None"])
...
# --- STEP 6: ANALYSIS ---
    ...
    if df_raw is None:
        ...
        st.markdown(
            f'<div class="status-box" style="background-color: {"#d4edda" if is_stable else "#f8d7da"};">{"STABLE" if is_stable else "UNSTABLE"}</div>',
            unsafe_allow_html=True)
        ...
        for i, target in enumerate(['Size_nm', 'PDI', 'Zeta_mV', 'Encapsulation_Efficiency']):
            ...
            with p_cols[i]:
                st.markdown(
                    f"<div class='metric-card'><div class='m-label'>{target}</div><div class='m-value'>{val:.2f}</div></div>",
                    unsafe_allow_html=True)
        ...
        with t1:
            ...
            fig_tern = go.Figure(go.Scatterternary(
                {'mode': 'markers', 'a': [st.session_state.get('oil_p', 15)], 'b': [st.session_state.get('smix_p', 30)],
                 'c': [st.session_state.get('water_p', 55)], 'marker': {'size': 20, 'color': 'green'}}))
